...
export async function configSet(key: string, value: unknown) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/wr_config`, {
    method: 'POST',
...
export async function getClients() {
  return dbGet('wr_clientes?select=*&order=nome.asc');
}
...
export async function getActiveProducts() {
  return dbGet('wr_produtos?select=*&ativo=eq.true&order=nome.asc');
}

export async function getRoutes() {
  const routes = await configGet('rotas');
  return Array.isArray(routes) && routes.length ? routes : ['Rota 01 - Zona Rural', 'Rota 02 - Centro', 'Rota 03 - Vila Nova'];
}
...
export function addressOf(client: any) {
  return [client?.rua, client?.numero, client?.bairro, client?.cidade, client?.estado].filter(Boolean).join(', ');
}
...
export async function sendMetaMessage(to: string, payload: Record<string, unknown>) {
  ...
  const response = await fetch(`https://graph.facebook.com/${META_GRAPH_VERSION}/${META_PHONE_NUMBER_ID}/messages`, {
    ...
    body: JSON.stringify({ messaging_product: 'whatsapp', to: destination, ...payload }),
  });
  const result = await response.json().catch(() => ({}));
  if (!response.ok) {
    const message = result?.error?.message || `WhatsApp retornou HTTP ${response.status}`;
...
export async function sendText(to: string, body: string) {
  return sendMetaMessage(to, { type: 'text', text: { preview_url: false, body } });
}
...
export async function sendWeeklyTemplate(to: string, clientName: string, deliveryDate: string) {
  return sendMetaMessage(to, {
    type: 'template',
    template: {
      name: META_WEEKLY_TEMPLATE,
      language: { code: META_WEEKLY_TEMPLATE_LANGUAGE },
      components: [{
        type: 'body',
        parameters: [
          { type: 'text', text: clientName || 'cliente' },
          { type: 'text', text: brDate(deliveryDate) },
        ],
      }],
    },
  });
...
function productAliases(product: any) {
  ...
  if (name.includes('500') || /\bqueijo p\b/.test(name) || name.includes('pequeno')) {
    ['queijo p', 'queijo pequeno', 'queijo 500g', 'queijo 500 g', '500g de queijo', '500 g de queijo'].forEach(value => aliases.add(value));
  }
  if (name.includes('1kg') || name.includes('1 kg') || /\bqueijo g\b/.test(name) || name.includes('grande')) {
    ['queijo g', 'queijo grande', 'queijo 1kg', 'queijo 1 kg', '1kg de queijo', '1 kg de queijo'].forEach(value => aliases.add(value));
  }
  ...
  for (const product of products) {
    ...
    if (quantity > 0) {
      items.push({
        ...
        total: quantity * Number(product.preco || 0),
      });
    }
  }
  return items;
