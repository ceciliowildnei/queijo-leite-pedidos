...
const loadFinancialConfig = `  async function loadRoutes() {
    const response = await supabase.from('wr_config').select('valor').eq('chave', 'rotas').maybeSingle();
    if (Array.isArray(response.data?.valor) && response.data.valor.length) {
      setRoutes(response.data.valor);
      writeLocal('wr_rotas', response.data.valor);
...
if (!source.includes('async function loadCashOuts()')) {
  if (!source.includes(loadRoutesBlock)) throw new Error('Função de carregamento das rotas não encontrada.');
  source = source.replace(loadRoutesBlock, loadFinancialConfig);
}
...
source = source.replace(
  /  function saveCashOut\(out\) \{[\s\S]*?\n  \}\n\n  function deleteCashOut\(id\) \{[\s\S]*?\n  \}/,
  cashFunctions
);

const financialControlPage = `function CashPage({ db, selectedDate, cashOuts, setModal, deleteCashOut }) {
  ...
  const movements = [
    ...paid.map(order => ({
      ...
      description: \`\${order.cliente_nome || 'Cliente'} · \${order.produto_nome || 'Produto'}\`,
      type: 'Entrada',
    ...
    ...pending.map(order => ({
      ...
      type: 'A receber',
      value: number(order.total),
      status: orderDate(order) < today ? 'Em atraso' : 'Pendente',
    })),
    ...periodOutputs.map(item => ({
      id: item.id,
      date: item.data,
      description: item.descricao || 'Saída',
      type: 'Saída',
  ...
  return (
    <div className="page-stack financial-control-page">
      <PageHeader
        ...
        subtitle="Visão completa do que entrou, saiu, está pendente e do saldo disponível."
        actions={<button className="btn btn-primary" onClick={() => setModal({ type: 'cash-out' })}>+ Lançar saída</button>}
      />
      ...
      <div className="metric-grid cash-metric-grid">
        <MetricCard label="Vendas do período" value={money(gross)} icon="R$" trend={\`\${periodOrders.length} lançamento(s)\`} tone="blue" />
        ...
        <MetricCard label="Saídas" value={money(expenses)} icon="↘" trend={\`\${periodOutputs.length} despesa(s)\`} tone="orange" />
      ...
      <div className="cash-health-grid">
        <Panel title="Situação dos recebimentos">
          <div className="cash-receipt-health">
            ...
            <div className="cash-progress-track"><span style={{ width: \`\${Math.min(receiptRate, 100)}%\` }} /></div>
        ...
        <Panel title="Entradas por forma de pagamento">
          <div className="cash-payment-list">
            {paymentBreakdown.length ? paymentBreakdown.map(item => (
              <div key={item.name}><span>{item.name}</span><small>{item.count} pagamento(s)</small><strong>{money(item.value)}</strong></div>
            )) : <div className="cash-empty-inline">Nenhuma entrada confirmada neste período.</div>}
          </div>
        </Panel>
      </div>

      <Panel title="Fluxo líquido dos últimos 14 dias">
        <BarChart data={flow} allowNegative />
      </Panel>

      <div className="cash-table-grid">
        <Panel title="Contas a receber">
          <DataTable compact columns={['Data', 'Cliente', 'Pedido', 'Forma', 'Situação', 'Valor']} rows={receivableRows} empty="Nenhuma conta pendente neste período." />
        </Panel>
        <Panel title="Saídas registradas">
          <DataTable compact columns={['Data', 'Descrição', 'Valor', 'Ações']} rows={outputRows} empty="Nenhuma saída registrada neste período." />
        </Panel>
      </div>
      ...
      <Panel title="Movimentações consolidadas">
        <DataTable
          compact
          columns={['Data', 'Descrição', 'Tipo', 'Valor', 'Status', 'Ações']}
          rows={movements.map(row => [
            brDate(row.date),
            row.description,
            row.type,
            <strong className={row.type === 'Saída' ? 'negative' : row.type === 'A receber' ? 'pending-value' : 'positive'}>{money(row.value)}</strong>,
            <Status value={row.status} />,
            row.removable ? <button className="danger-link" onClick={() => deleteCashOut(row.id)}>Excluir</button> : '—',
          ])}
          empty="Nenhuma movimentação financeira neste período."
        />
      </Panel>
    </div>
  );
...
source = source.replace(
  /function CashPage\([\s\S]*?\n}\n\nfunction ReportsPage/,
  `${financialControlPage}\n\nfunction ReportsPage`
);

if (!source.includes('Saldo geral disponível')) throw new Error('Controle financeiro completo não foi aplicado.');
