...
source = source.replace(clientFormOriginal, clientFormSimple);
source = source.replace(clientFormExpanded, clientFormSimple);

if (!source.includes(clientFormSimple)) {
  throw new Error('Não foi possível aplicar o cadastro simples de cliente.');
}
...
console.log('Cadastro simples de cliente e relatório de separação preparados.');
