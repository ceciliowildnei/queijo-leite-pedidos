...
const loadRoutesBlock = `  async function loadRoutes() {
...
const loadRoutesAndCash = `  async function loadRoutes() {
    const response = await supabase.from('wr_config').select('valor').eq('chave', 'rotas').maybeSingle();
    if (Array.isArray(response.data?.valor) && response.data.valor.length) {
      setRoutes(response.data.valor);
      writeLocal('wr_rotas', response.data.valor);
...
if (!source.includes('async function loadCashOuts()')) {
  if (!source.includes(loadRoutesBlock)) throw new Error('Função de rotas não encontrada.');
  source = source.replace(loadRoutesBlock, loadRoutesAndCash);
}
...
const cashFunctions = `  async function saveCashOut(out) {
    ...
    addLog('Saída lançada', String(out.descricao || 'Saída') + ' · ' + money(out.valor));
...
source = source.replace(
  /  (?:async )?function saveCashOut\(out\) \{[\s\S]*?\n  \}\n\n  (?:async )?function deleteCashOut\(id\) \{[\s\S]*?\n  \}/,
  cashFunctions
);

const cashPage = `function CashPage({ db, selectedDate, cashOuts, setModal, deleteCashOut }) {
  ...
  const periodOrders = activeOrders.filter(order => inPeriod(orderDate(order)));
  const periodOutputs = cashOuts.filter(item => inPeriod(item.data));
  ...
  const allPaid = activeOrders.filter(isPaid);
  const allPending = activeOrders.filter(order => !isPaid(order));
  const allReceived = sum(allPaid, 'total');
  const allReceivable = sum(allPending, 'total');
  const allExpenses = sum(cashOuts, 'valor');
  const allBalance = allReceived - allExpenses;
  const allGross = sum(activeOrders, 'total');

  const monthLabelRaw = new Intl.DateTimeFormat('pt-BR', { month: 'long', year: 'numeric' }).format(dateValue(selectedDate || today));
  ...
  const receivableRows = [...pending]
    .sort((a, b) => String(orderDate(a)).localeCompare(String(orderDate(b))))
    .map(order => [
      brDate(orderDate(order)),
      order.cliente_nome || '—',
      order.codigo || '—',
  ...
  const outputRows = [...periodOutputs]
    .sort((a, b) => String(b.data || '').localeCompare(String(a.data || '')))
    .map(item => [
      brDate(item.data),
      item.descricao || 'Saída',
      <strong className="negative">{money(item.valor)}</strong>,
  ...
  const movements = [
    ...
    ...periodOutputs.map(item => ({ id: item.id, date: item.data, description: item.descricao || 'Saída', type: 'Saída', value: -number(item.valor), status: 'Despesa', removable: true })),
  ...
  return (
    <div className="page-stack financial-control-page">
      ...
      <section className="cash-overview-hero" aria-label="Resumo financeiro geral">
        <div className="cash-overview-primary">
          <span>Saldo geral disponível</span>
          <strong className={allBalance < 0 ? 'negative' : ''}>{money(allBalance)}</strong>
          <small>Total recebido menos todas as saídas registradas.</small>
        </div>
        <div className="cash-overview-breakdown">
          <div><span>Total vendido</span><strong>{money(allGross)}</strong></div>
          ...
          <div><span>Cancelados</span><strong>{cancelledOrders.length}</strong></div>
        </div>
      </section>

      <div className="cash-period-control" role="tablist" aria-label="Período do controle financeiro">
        <div><span>Período analisado</span><strong>{periodLabel}</strong></div>
        <div className="cash-period-tabs">
      ...
      <div className="metric-grid cash-metric-grid">
        <MetricCard label="Vendas do período" value={money(gross)} icon="R$" trend={String(periodOrders.length) + ' lançamento(s)'} tone="blue" />
        <MetricCard label="Entradas confirmadas" value={money(received)} icon="↗" trend={String(paid.length) + ' recebido(s)'} tone="green" />
        <MetricCard label="A receber" value={money(receivable)} icon="◷" trend={String(pending.length) + ' pendente(s)'} tone="gold" />
        <MetricCard label="Saídas" value={money(expenses)} icon="↘" trend={String(periodOutputs.length) + ' despesa(s)'} tone="orange" />
      ...
      <div className="cash-health-grid">
        <Panel title="Situação dos recebimentos">
          <div className="cash-receipt-health">
            <div className="cash-progress-header"><span>Percentual já recebido</span><strong>{receiptRate}%</strong></div>
            <div className="cash-progress-track"><span style={{ width: String(Math.min(receiptRate, 100)) + '%' }} /></div>
            <div className="cash-status-list">
              <div><span>Recebidos</span><strong>{paid.length}</strong><small>{money(received)}</small></div>
              <div><span>Pendentes</span><strong>{pending.length}</strong><small>{money(receivable)}</small></div>
              <div className={overdue.length ? 'attention' : ''}><span>Em atraso</span><strong>{overdue.length}</strong><small>{money(sum(overdue, 'total'))}</small></div>
              <div><span>Vencem hoje</span><strong>{dueToday.length}</strong><small>{money(sum(dueToday, 'total'))}</small></div>
              <div><span>Futuros</span><strong>{futurePending.length}</strong><small>{money(sum(futurePending, 'total'))}</small></div>
            </div>
          </div>
        </Panel>

        <Panel title="Entradas por forma de pagamento">
          <div className="cash-payment-list">
            {paymentBreakdown.length ? paymentBreakdown.map(item => <div key={item.name}><span>{item.name}</span><small>{item.count} pagamento(s)</small><strong>{money(item.value)}</strong></div>) : <div className="cash-empty-inline">Nenhuma entrada confirmada neste período.</div>}
          </div>
        </Panel>
      </div>
      ...
      <div className="cash-table-grid">
        <Panel title="Contas a receber"><DataTable compact columns={['Data', 'Cliente', 'Pedido', 'Forma', 'Situação', 'Valor']} rows={receivableRows} empty="Nenhuma conta pendente neste período." /></Panel>
        <Panel title="Saídas registradas"><DataTable compact columns={['Data', 'Descrição', 'Valor', 'Ações']} rows={outputRows} empty="Nenhuma saída registrada neste período." /></Panel>
      </div>

      <Panel title="Movimentações consolidadas">
        <DataTable compact columns={['Data', 'Descrição', 'Tipo', 'Valor', 'Status', 'Ações']} rows={movements.map(row => [brDate(row.date), row.description, row.type, <strong className={row.type === 'Saída' ? 'negative' : row.type === 'A receber' ? 'pending-value' : 'positive'}>{money(row.value)}</strong>, <Status value={row.status} />, row.removable ? <button className="danger-link" onClick={() => deleteCashOut(row.id)}>Excluir</button> : '—'])} empty="Nenhuma movimentação financeira neste período." />
      </Panel>
    </div>
  );
}`;

source = source.replace(/function CashPage\([\s\S]*?\n}\n\nfunction ReportsPage/, cashPage + '\n\nfunction ReportsPage');

if (!source.includes('Saldo geral disponível')) throw new Error('Controle financeiro não foi aplicado.');
if (!source.includes("eq('chave', 'cash_outs')")) throw new Error('Sincronização das saídas não foi aplicada.');
